function [T_all, mean_dist, history] = ukf_isotropic_registration_ex2(moving, fixed, varargin)
...
%                  that will be moving towards the fixed point set (the target).
%
...
%
%   movingnormal   A 3xN matrix with each column represent the number of
%                  measurement/observation within the set. This point set
%                  is normal vectors which corresponds to the 'moving'
%                  parameter.
%
%   fixednormal    A 3xM matrix with each column represent the number of
%                  measurement/observation within the set. This point set
%                  is normal vectors which corresponds to the 'fixed'
%                  parameter.
%
%   normalratio    A double. To make normal values comparable with position
%                  values.
%
...
%
%   display        A logical. If you want to display the progress of the 
%                  registration, set this parameter to true. If you also 
%                  want to see the groundtruth point set transforming 
%                  towards to fixed point set, you should specify it the 
%                  "movingdebug" parameter.
%
% OUTPUT:
% 
...
addParameter( p, 'movingnormal', zeros(3,1), ...
              @(x) validateattributes(x, {'double'}, {'nrows', 3}) );
addParameter( p, 'fixednormal', zeros(3,1), ...
              @(x) validateattributes(x, {'double'}, {'nrows', 3}) );
...
% if user specified normals
if( and(any(p.Results.movingnormal, 'all'), any(p.Results.fixednormal, 'all') ) )
    normal_specified = true;
    U_normals        = p.Results.movingnormal;
    Y_breve_normals  = p.Results.fixednormal;
else
    normal_specified = false;
end
...
if(p.Results.display)
    ...
    if(any(p.Results.movingdebug, 'all'))
        plot3( axes1, ...
               U_breve(1,:), ...
               U_breve(2,:), ...
               U_breve(3,:), ...
...
% registration variable initialization
U_breve_transformed = U_breve;
U_transformed       = U;
if(normal_specified)
	U_normals_transformed = U_normals;
end
...
for iter=1:max_iter
	...
	% In this simulation, the correspondences is unknown. There are
	% multiple approach to look for correspondences. In the original paper,
    % they used closest point operator. Here, i put a different option, if 
    % the user specified the normals 'movingnormal' and 'fixednormal', we 
    % will use the position and normal similarity.
    if(normal_specified)
        % get the normal data
        U_k_normals     = U_normals_transformed(:, rand_idx(1:point));        
        
        % setting up 6d points (position and orientation)
        % normal_ratio = 0.05;
        normal_ratio = p.Results.normalratio;
        Y_breve_withnormals = [Y_breve; Y_breve_normals * normal_ratio];
        U_k_withnormals     = [U_k; U_k_normals * normal_ratio];
        
        % kd-tree search in 6d space
        [idx, ~]  = knnsearch(Y_breve_withnormals', U_k_withnormals', 'Distance', 'euclidean');
        Y_k       = Y_breve(:, idx);
        mean_dist = mean(sqrt(sum((Y_k - U_k).^2, 1)));
        
    % If the otherwise, we will use the position similarity, i.e. closest
    % point operator.
    else
    ...
    for k=1:point
        ...
        temp           = repelem({1.0 * Sigma_yacute}, k);
        Sigma_yacute_k = blkdiag(temp{:});
        temp           = repelem({10.0 * mean_dist * eye(3)}, k);
    ...
    % use the estimated transformation parameters (xest_k) to update the
    % moving dataset (U)
    ...
    % if user specified normals transform the U normals
    if(normal_specified)
        U_normals_transformed = R_xest_bar_theta * U_normals_transformed + xest_bar_trans;
    end
